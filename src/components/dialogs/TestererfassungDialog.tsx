...
interface TestererfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Testererfassung['fields']) => Promise<void>;
  defaultValues?: Testererfassung['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function TestererfassungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: TestererfassungDialogProps) {
  const [fields, setFields] = useState<Partial<Testererfassung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'testererfassung');
      await onSubmit(clean as Testererfassung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Testererfassung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Edit Testererfassung' : 'New Testererfassung';
