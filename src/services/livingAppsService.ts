...
import type { Testererfassung, CreateTestererfassung } from '@/types/app';
...
export class LivingAppsService {
  // --- TESTERERFASSUNG ---
  static async getTestererfassung(): Promise<Testererfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TESTERERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Testererfassung[];
    return enrichLookupFields(records, 'testererfassung');
  }
  static async getTestererfassungEntry(id: string): Promise<Testererfassung | undefined> {
    ...
    const record = { record_id: data.id, ...data } as Testererfassung;
    return enrichLookupFields([record], 'testererfassung')[0];
  }
  static async createTestererfassungEntry(fields: CreateTestererfassung) {
    return callApi('POST', `/apps/${APP_IDS.TESTERERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'testererfassung') });
  }
  static async updateTestererfassungEntry(id: string, fields: Partial<CreateTestererfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TESTERERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'testererfassung') });
  }
  static async deleteTestererfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TESTERERFASSUNG}/records/${id}`);
  }
