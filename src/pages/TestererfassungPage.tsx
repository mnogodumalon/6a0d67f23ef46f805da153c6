...
export default function TestererfassungPage() {
  ...
  const [viewingRecord, setViewingRecord] = useState<Testererfassung | null>(null);
  ...
  return (
    <PageShell
      title="Testererfassung"
      subtitle={`${records.length} Testererfassung in the system`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Search Testererfassung..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-16 text-muted-foreground">
                  {search ? 'No results found.' : 'No Testererfassung yet. Add one!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
