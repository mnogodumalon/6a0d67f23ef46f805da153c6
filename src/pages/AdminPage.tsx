...
import { TestererfassungViewDialog } from '@/components/dialogs/TestererfassungViewDialog';
...
const TESTERERFASSUNG_FIELDS = [
  ...
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'option_a', label: 'Option A' }, { key: 'option_b', label: 'Option B' }, { key: 'option_c', label: 'Option C' }] },
  { key: 'bemerkungen', label: 'Bemerkungen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'testererfassung', label: 'Testererfassung', pascal: 'Testererfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('testererfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'testererfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testererfassung': return (data as any).testererfassung as Testererfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testererfassung': return TESTERERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testererfassung': return {
        create: (fields: any) => LivingAppsService.createTestererfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTestererfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTestererfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'testererfassung' || dialogState?.entity === 'testererfassung') && (
        <TestererfassungDialog
          open={createEntity === 'testererfassung' || dialogState?.entity === 'testererfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'testererfassung' ? handleUpdate : (fields: any) => handleCreate('testererfassung', fields)}
          defaultValues={dialogState?.entity === 'testererfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Testererfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Testererfassung']}
        />
      )}
      {viewState?.entity === 'testererfassung' && (
        <TestererfassungViewDialog
          open={viewState?.entity === 'testererfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'testererfassung', record: r }); }}
        />
      )}
